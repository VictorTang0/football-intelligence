  ...
  function renderHistoryRecords(historyData) {
    ...
    Object.keys(groups).sort((a, b) => new Date(b) - new Date(a)).forEach(date => {
      ...
      groups[date].forEach(r => {
        ...
        const recHtml = `<div style="margin-bottom:3px;"><span style="color:var(--text-3);font-weight:500;">方向:</span> <span style="font-weight:600; color:${recCorrect ? 'var(--rose, #f43f5e)' : 'var(--text-2)'};">${recommendationVal}</span>${recCorrect ? badgeRed : badgeBlack}</div>`;
