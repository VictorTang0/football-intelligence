              ...
              <div style="grid-column: 1 / -1; background:rgba(168, 85, 247, 0.08); border:1px solid rgba(168, 85, 247, 0.3); padding:10px 14px; border-radius:8px; margin-top:6px;">
                ...
                <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap:8px; font-size:11.5px;">
                  <div style="background:rgba(0,0,0,0.25); padding:6px 10px; border-radius:6px; border:1px solid rgba(255,255,255,0.05);">
                    <div style="color:var(--text-3); font-size:10.5px; margin-bottom:2px;">5000场极速胜率</div>
  ...
  function renderHistoryRecords(historyData) {
    ...
    if (sortedDatesDesc.length > 0) {
      function renderM10DayRows(dayTag) {
        ...
        const cleanText = (txt) => {
          if (!txt || txt === '--' || txt === 'None') return '无推荐';
          const t = String(txt).replace(/[\(（]竞彩[\)）]/g, '').trim();
          return t || '无推荐';
        };
            ...
            <div style="padding:8px 12px; border-bottom:1px solid rgba(255,255,255,0.05); margin-bottom:5px; border-radius:6px; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:8px; font-size:12.5px; ${rowHighlight}">
              <div style="display:flex; align-items:center; gap:8px; min-width:210px;">
                ...
                <div>
                  ...
                  ${m10 && m10.m10_bold_score && m10.m10_bold_score !== '无' ? `<div style="font-size:11px; color:#fbbf24; font-weight:bold; margin-top:3px; background:rgba(245,158,11,0.12); padding:1px 6px; border-radius:4px; border:1px solid rgba(245,158,11,0.3); display:inline-block;">${m10.m10_bold_score}</div>` : ''}
