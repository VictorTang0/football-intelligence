...
def enrich_h2h_and_form():
    ...
    for m in matches_db.get("matches", []):
        ...
        if paper_gap >= 3.5 and (h2h_h_score >= 7.5 or "博德" in home):
            ...
            m["ultimate_conclusion"]["reasoning"] = f"从中国竞彩网 (https://www.sporttery.cn/jc/zqszsc/) 即时抓取信息来看，{home} 在交锋与纸面硬实力（评分 {h_paper} 对 {a_paper}）上呈现绝对碾压态势。模型判定本场为高度稳健的主胜格局。"
