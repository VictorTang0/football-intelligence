...
5. **交锋与近期战绩权威自检协议**：严禁伪随机战绩生成函数 (如 gen_local)；近期战绩必须来自竞彩官方开奖真理库，并严格按日期降序排列提取最新 5 场；若竞彩网缺少积分，从全量开奖库中实时动态聚合估算。

## 7. Data Authenticity and Verification
在处理任何赛事数据时，分析师模型必须执行以下真实性校验：
1. **数据源权威性一致性校验**：所有涉及 `team_stats` 和 `history_results` 的数据，必须强制调用最新 API 获取，严禁使用过时缓存；当本地缓存数据与最新 API 返回存在冲突时，强制触发 `force-refresh` 机制以 API 为准。
2. **关键比分审计**：任何在 `results` 字段中更新的历史赛事，必须通过至少两个独立数据来源（如官方联赛 API 与权威博彩数据源）进行比对验证，严禁只依赖单一信源更新。
